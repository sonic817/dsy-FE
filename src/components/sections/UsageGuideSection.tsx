...
export default function UsageGuideSection() {
  ...
  useEffect(() => {
    if (activeTab !== 0 || isProgramModalOpen || programs.length <= 1) return;
    const interval = setInterval(() => {
      setSelectedProgram((current) => {
        if (!current) return programs[0];
        const idx = programs.findIndex((p) => p.id === current.id);
        return programs[(idx + 1) % programs.length];
      });
    }, 3000);
    return () => clearInterval(interval);
  }, [activeTab, isProgramModalOpen, programs]);
